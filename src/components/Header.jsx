...
function Header() {
  ...
  const linkClass = "relative transition hover:text-[#d58b2a] focus:outline-none focus:text-[#d58b2a]"
  ...
  return (
    <header className="fixed left-0 top-0 z-50 w-full border-b border-[#fff8c9]/20 bg-[#1f0d07]">
      <div className="mx-auto flex h-20 max-w-[1600px] items-center justify-between px-6 md:px-8">
        ...
        >
          <span className={`h-[2px] w-5 bg-[#fff8c9] transition ${isMenuOpen ? "translate-y-2 rotate-45" : ""}`} />
          <span className={`h-[2px] w-5 bg-[#fff8c9] transition ${isMenuOpen ? "opacity-0" : ""}`} />
          <span className={`h-[2px] w-5 bg-[#fff8c9] transition ${isMenuOpen ? "-translate-y-2 -rotate-45" : ""}`} />
        </button>
      </div>
      ...
      {isMenuOpen && (
        <div className="absolute left-0 top-20 z-40 w-full border-b border-[#fff8c9]/30 bg-[#1f0d07] px-6 py-8 md:hidden">
          <nav className="flex flex-col gap-6 text-3xl uppercase text-[#fff8c9]">
            <NavLink to="/" onClick={closeMenu} className={({ isActive }) => `${linkClass} ${isActive ? activeClass : ""}`}>Главная</NavLink>
            <NavLink to="/catalog" onClick={closeMenu} className={({ isActive }) => `${linkClass} ${isActive ? activeClass : ""}`}>Каталог</NavLink>
            <NavLink to="/cart" onClick={closeMenu} className={({ isActive }) => `${linkClass} ${isActive ? activeClass : ""}`}>Корзина {totalCount > 0 && `(${totalCount})`}</NavLink>
            <NavLink to="/profile" onClick={closeMenu} className={({ isActive }) => `${linkClass} ${isActive ? activeClass : ""}`}>Профиль</NavLink>
          </nav>
        </div>
      )}
    </header>
